...
class GuessTheNumberAskingQuestionsExplicitModelTask(Task):
    ...
    def give_instructions(self, event):
        """ picking a random nuber of digits between 1 and 5. generating a random number with that number of digits.
         first value shouldn't be 0, although this doesn't really matter for our current purposes. on weird limit
         properties of Python's range  preparing a regexp to capture requests for help we need to escape the periods
         and question marks in number_questions
...
class GuessTheNumberAskingQuestionsTask(Task):
    ...
    def give_instructions(self, event):
        ...
        first value shouldn't be 0, although this doesn't really.  matter for our current purposes.  on weird limit
        properties of Python's range preparing a regexp to capture requests for help we need to escape the periods
        and question marks in number_questions preparing the message
    ...
    def give_away_answer(self,event):
        ...
        """
        self.set_message(
            'if you asked: ' + random.choice(number_questions) + ', I would have said: '+ self.target_number + '.')
...
class GuessTheNumberAskingForDigitsTask(Task):
    ...
    def give_instructions(self, event):
        ...
        regular expression.  picking a random nuber of digits between 1 and 5.  generating a random number with that
        number of digits.  first value shouldn't be 0, although this doesn't really matter for our current purposes
         this relies weird limit properties of Python's range preparing a regexp to capture requests for help.  also,
         we initialize a counter to keep track of the next digit preparing the message
        :param event:
        :return:
        """
        ...
        self.next_digit=0
        message_string = "guess the " + str(
            self.digits) + "-digit number I am thinking of; you can ask me for the next digit."
        self.set_message(message_string)
        self.instructions_completed = False
...
"""
OLD STUFF FROM HERE here, I define a global character-by-character association list that can be used by the tasks 
below that rely on the same association scheme (here and below, global means: accessible by all tasks in this file; 
local means: accessible within one task only) we select a subset of characters as primes, so we can also define tasks 
with disjoint primes from within and without this list the following global variable tells us the size of this subset:
"""
global_prime_cardinality=5

alphabet_integers=list(range(0,26))  # weirdly, left value is inclusive, right value is exclusive
...
""" the following function returns instead matching primes and targets tuples, generating them each time it is 
called: it will be used by "local" tasks to generate their own mapping tables (note that objects returned are two 
TUPLES, as needed by the task classes):
"""
def generate_local_prime_and_target_mappings(prime_cardinality):
    """ weirdly, left value is inclusive, right value is exclusive usual python silliness: this will range from 0 to
    prime_cardinality-1. we must fix to tuple, or else class will break down primes=tuple(primes) # usual python
    weirdness# we must fix to tuple, or else class will break down # also deleting alphabet_integers
...
class LocalCharacterPrimeTarget(Task):
    """ get local primes and targets. note that we use the same number of distinct primes as in the global.  table,
    but they are not constrained to be the same (nor to be disjoint)
