...
class MiniTasksAutomaton(object):
    ...
    def _get_string(self, threshold_length):
        ...
        the function is_string_correct().# the symbols get confirmed up to current when a final state is visited #
        when doing "and" generation, all final states must be visited # will hold the generated string# useful when
        doing "and" generation and we want to force visiting certain final state # until the string is generated #
        insert an "anything" string into the generated string (last_confirmed == current) is true after a final state
        is entered when "not" strings are used, there's always "anything" allowed (asserted above) # random: do not
         insert the "anything" strings always# when finishing generating the "and" strings, force-visit the missing
         final states# the states that correspond to one n-gram form a sequence:# try to reach missing states when in
         "and" mode find a symbol that will lead to one of the missing states (if there is such a symbol)# finish
         generating only after generating the rest of the last n-gram
        ...
        while True:
            if last_confirmed == current and self.anything_allowed and current < threshold_length:
                if random.random() > 0.5:
                ...
                if current >= threshold_length:
                    if len(missing_states) > 0 and self.logical_op == self._and_string:
                        ...
                        if self.anything_allowed \
                                or self._is_in_final_state_with_sufficient_depth(
                                    automaton, state, self.positive_state_depth_dict, current-last_confirmed+1):
                            break
    ...
    def _get_random_wrong_string(self, string_length):
        """ Try 3 times to generate a random string not accepted by the function is_string_correct# raise
        AssertionError("could not generate a random string that is not accepted by the automaton")
    ...
    def _get_almost_correct_string(self, length):
        ...
        4: not case: either take the strategy from above or insert a bad not string (50/50)
         # can happen if there's just "anything" as the language description # task 1 and 2 and 3 # task 3 or 4# we
         cannot move the ngram to the negative table, because the negative table must be drawn from a separate alphabet
        ...
        if not self.anything_allowed:
            if random.random() > 0.5:
                insert_single_bad_positive_character = True
            else:
                insert_bad_character = True
        if self.logical_op == self._and_string and len(self.positive_table) >= 2:
            if not self.anything_allowed and random.random() > 0.5:
                insert_single_bad_positive_character = False
                insert_bad_character = False
            remove_one_ngram = True
        if self.logical_op == self._and_string and self.anything_allowed and len(self.positive_table) >= 1:
            remove_one_ngram = True
    ...
    def _eval_positive(self, string):
        """ Verify if the positive NFA recognises given word. # loop here to be a matcher automaton# > condition is
        true if there's more than one match # there is no unmatched character in word up to position i# there is an
        unmatched character at position last_confirmed + 1
    ...
    def _eval_negative(self, string):
        """
        Verify if the negative NFA matches any ngram in the given word. # loop here to be a matcher automaton
