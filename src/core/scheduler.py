...
class ConsecutiveTaskScheduler:
    """ Switches to the next task type sequentially After the current task was successfully solved N times in row
    ...
    def get_next_task(self):
        """ you can insert some handler code here.
        ...
        if self.reward_count >= self.success_threshold:
            ...
            if self.task_ptr >= len(self.tasks):
                ...
                sys.exit()   # TODO code unreachable
                return None
        return self.tasks[self.task_ptr]
...
class DependenciesTaskScheduler:
    """ Takes a dependency graph between the tasks and randomly allocates between the ones that are at the root,
    or are dependent on other tasks that have been solved (based on a threshold)

    """
    def __init__(self, tasks, tasks_dependencies, unlock_threshold=10):
        """# a dictionary containing rewards per task# saves the last task that has been given to the learner# set
        of the tasks that are available to the learner# initially these are the tasks that have no dependencies on them

        :param tasks:  a list of Task objects
        :param tasks_dependencies: list of ordered pairs of Task objects (t1,t2) if t2 depends on t1 to be completed.
